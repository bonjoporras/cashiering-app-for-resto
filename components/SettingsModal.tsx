...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'database'>(initialTab);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setImportStatus('idle'); // Reset status on open
    }
  }, [isOpen, initialTab]);
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      setImportStatus('idle'); // Clear previous status immediately
      
      // Delay confirm slightly to ensure UI updates if needed, though mostly for cleaner event loop
      setTimeout(async () => {
        ...
        // CRITICAL: Always reset the input value so the same file can be selected again if it failed or was cancelled
        e.target.value = ''; 
      }, 50);
    }
  };
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl flex items-start gap-3 border border-blue-100 dark:border-blue-900/30">
                   ...
                   <div>
                      ...
                      <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                        Securely backup or restore your entire system data. <br/>
                        Includes: <strong>Inventory, Sales History, Customers, Users, and Settings.</strong>
                      </p>
                   </div>
                </div>

                <div className="space-y-3">
                   <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Backup & Restore</label>
                   ...
                   >
                      <div className="flex items-center gap-3">
                         ...
                         <div className="text-left">
                            <p className="font-bold text-slate-800 dark:text-white text-sm">Backup All Data</p>
                            <p className="text-xs text-slate-500 dark:text-slate-400">Download full database backup (.json)</p>
                         </div>
                      </div>
                   </button>

                   <label className={`w-full flex items-center justify-between p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-blue-500 hover:shadow-md transition-all cursor-pointer group relative overflow-hidden ${importStatus !== 'idle' ? 'pointer-events-auto' : ''}`}>
                      <div className="flex items-center gap-3">
                         <div className="bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 p-2 rounded-lg">
                            <Upload size={20} />
                         </div>
                         <div className="text-left">
                            <p className="font-bold text-slate-800 dark:text-white text-sm">Restore All Files</p>
                            <p className="text-xs text-slate-500 dark:text-slate-400">Import backup file to restore system</p>
                         </div>
                      </div>
                      <input type="file" accept=".json" onChange={handleImport} className="hidden" />
                      
                      {importStatus === 'success' && (
                        <div className="absolute inset-0 bg-green-500 text-white flex items-center justify-center font-bold text-sm z-10">
                           <CheckCircle size={18} className="mr-2" /> Restored Successfully! Reloading...
                        </div>
                      )}
                      {importStatus === 'error' && (
                        <div className="absolute inset-0 bg-red-500 text-white flex items-center justify-center font-bold text-sm z-10 cursor-pointer" title="Click to try again">
                           <AlertTriangle size={18} className="mr-2" /> Failed! Invalid Backup File. (Click to retry)
                        </div>
                      )}
                   </label>
                </div>
